<template>
  <div>
    <h2 class="text-3xl font-medium"> {{ editMode ? "Update " : "Add New " }} Subject</h2>
...
export default {
    ...
    return {
      ...
      typeOptions: [
        { name: "Select Subject Type", value: "" },
          ...
          { name: "Optional for all", value: "optional-for-all" },
          { name: "Optional for science", value: "optional-for-science" },
          { name: "Optional for humanities", value: "optional-for-humanities" },
        ],
  ...
  computed: {
    ...mapState(["aSubject"]),
    ...mapGetters(["is", "isError", "getASubject"]),
  },

  methods: {
    ...mapActions(["postCreateSubject", "fetchASubject", "updateASubject"]),
    ...
    async uploadASubject() {
      ...
      if (!this.$v.subject.$invalid) {
        if (this.editMode) {
          await this.updateASubject(this.subject);
        } else {
  ...
  watch: {
    'getASubject': function(v) {
      this.subject = JSON.parse(JSON.stringify(v))
    }
  },
  ...
  mounted() {
    if (this.$route.params && this.$route.params.id) {
      ...
      this.editMode = true;
      this.fetchASubject(this.subjectID)
      this.subject = JSON.parse(JSON.stringify(this.getASubject))
    }
  },
};
</script>
